...
def sim_tournament():
    ...
    for g in group_keys:
        ...
        thirds.append((third, stats_all[g][third]["pts"],
                       ...
                       ELO.get(third,1500)))
    best8 = [t[0] for t in sorted(thirds,
             key=lambda x:(x[1],x[2],x[3],x[4]), reverse=True)[:8]]

    # Build R32 bracket: pair group winners vs runners-up cross-group
    # Simplified sequential pairing
    r32 = []
    for g in group_keys:
...
for i in range(N_SIMULATIONS):
    ...
    # Winner also reached SF and Final
    if winner in counts:
        counts[winner]["final"] = counts[winner].get("final",0)
        counts[winner]["sf"]    = counts[winner].get("sf",0)
